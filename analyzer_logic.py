...
def get_config_path():
    """Returns the path to the config file, located next to the script."""
    try:
...
def parse_prg_file(filename):
    """
    Parses a .prg file and converts it into a structured list of segments.
    This version uses a two-pass approach for robust feature number tracking.
    """
    feature_map = []
    try:
        ...
        messagebox.showerror("Error", f"File not found during parsing: {filename}")
        return []
    except Exception as e:
        messagebox.showerror("File Read Error", f"Could not read the file. Error: {e}")
        return []
...
def run_path_stress_analysis(parsed_segments, g_factor):
    ...
    for i in range(len(parsed_segments) - 1):
        ...
        if current_seg['type'] in ['PTP', 'LINE'] and next_seg['type'] in ['PTP', 'LINE']:
            v1_x = current_seg['x'] - current_seg['start_x']; v1_y = current_seg['y'] - current_seg['start_y']
            v2_x = next_seg['x'] - next_seg['start_x']; v2_y = next_seg['y'] - next_seg['start_y']
            mag_v1 = math.sqrt(v1_x**2 + v1_y**2); mag_v2 = math.sqrt(v2_x**2 + v2_y**2)
        ...
        if current_seg['type'] in ['PTP', 'LINE'] and next_seg['type'] == 'ARC2':
            v_line_x = current_seg['x'] - current_seg['start_x']; v_line_y = current_seg['y'] - current_seg['start_y']
            v_rad_x = next_seg['start_x'] - next_seg['center_x']; v_rad_y = next_seg['start_y'] - next_seg['center_y']
            mag_line = math.sqrt(v_line_x**2 + v_line_y**2); mag_rad = math.sqrt(v_rad_x**2 + v_rad_y**2)
            if mag_line > 1e-6 and mag_rad > 1e-6 and abs(((v_line_x * v_rad_x) + (v_line_y * v_rad_y)) / (mag_line * mag_rad)) > TANGENT_DOT_PRODUCT_TOLERANCE:
                stress_events.append({'type': 'Line-Arc Stress', 'line_num': next_seg['line_num'], 'feature_num': next_seg['feature_num'],
                                      'coords': (next_seg['start_x'], next_seg['start_y']),
        ...
        if current_seg['type'] == 'ARC2' and next_seg['type'] in ['PTP', 'LINE']:
            ...
            l_x = next_seg['x'] - next_seg['start_x']; l_y = next_seg['y'] - next_seg['start_y']
            mag_t = math.sqrt(t_x**2 + t_y**2); mag_l = math.sqrt(l_x**2 + l_y**2)
            if mag_t > 1e-6 and mag_l > 1e-6 and abs(abs(((t_x * l_x) + (t_y * l_y)) / (mag_t * mag_l)) - 1.0) > TANGENT_DOT_PRODUCT_TOLERANCE:
                stress_events.append({'type': 'Line-Arc Stress', 'line_num': next_seg['line_num'], 'feature_num': next_seg['feature_num'],
                                      'coords': (next_seg['start_x'], next_seg['start_y']),
        ...
        if current_seg['type'] == 'ARC2' and next_seg['type'] == 'ARC2':
            ...
            mag_t1 = math.sqrt(t1_x**2 + t1_y**2); mag_t2 = math.sqrt(t2_x**2 + t2_y**2)
            if mag_t1 > 1e-6 and mag_t2 > 1e-6:
                ...
                if angle > CRITICAL_TURNING_ANGLE_DEG and angle < (180 - CRITICAL_TURNING_ANGLE_DEG):
                    stress_events.append({'type': 'Arc-to-Arc Stress', 'line_num': next_seg['line_num'], 'feature_num': next_seg['feature_num'],
                                          'coords': (next_seg['start_x'], next_seg['start_y']),
...
def generate_analysis_report(limiting_speed, stress_events, g_factor, limiting_arc_details):
    ...
    if limiting_speed:
        ...
        if limiting_arc_details:
            report_lines.append(f" -> This speed is limited by an arc in Feature {limiting_arc_details['feature_num']}.")
    else:
...
def create_annotated_prg_file(original_filename, annotated_filename, limiting_speed, stress_events, arc_info_events,
    ...
    try:
        ...
        messagebox.showerror("File Write Error", f"Could not create annotated file. Error: {e}")
...
def animate_printer(filename_to_simulate, limiting_speed):
    segments = parse_prg_file(filename_to_simulate)
    if not segments:
        messagebox.showinfo("Info", "No drawable segments found for animation.")
        return
    ...
    if not all_x or not all_y:
        ax.set_xlim(-5, 10); ax.set_ylim(-2, 10)
    else:
    ...
    ax.set_aspect('equal', adjustable='box'); ax.grid(True, linestyle='--', alpha=0.7)
    ...
    def update(frame_idx):
        ...
        if seg['type'] == 'ARC2':
            ...
            if arc_pts:
                ax.plot([p[0] for p in arc_pts], [p[1] for p in arc_pts], 'r-' if seg['is_printing'] else 'b--', linewidth=2.0 if seg['is_printing'] else 1.2, alpha=0.8)
                end_x, end_y = arc_pts[-1]
        elif 'x' in seg:
            ax.plot([seg['start_x'], seg['x']], [seg['start_y'], seg['y']], 'r-' if seg['is_printing'] else 'b--', linewidth=2.0 if seg['is_printing'] else 1.2, alpha=0.8)
            end_x, end_y = seg['x'], seg['y']


        if end_x is not None: head_dot.set_data([end_x], [end_y])

        speed_display = f"{limiting_speed:.1f} mm/s (Limit)" if seg['is_printing'] and limiting_speed is not None else "Default Rapid"
        status_text = "Printing" if seg['is_printing'] else "Rapid Move"
        info_text.set_text(f"Feature: {seg.get('feature_num', 'N/A')}\n{status_text}\nSpeed: {speed_display}")
        return [head_dot, info_text]

    ani = animation.FuncAnimation(fig, update, frames=len(segments) + 1, init_func=init, blit=False, interval=50, repeat=False)
    plt.tight_layout()
    plt.show()
    return ani